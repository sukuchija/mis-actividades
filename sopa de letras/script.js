...
async function generateWordDoc() {
    ...
    try {
        ...
        // Leer el archivo de forma segura usando FileReader en una promesa
        const templateArrayBuffer = await new Promise((resolve, reject) => {
            const reader = new FileReader();
            reader.onload = (e) => resolve(e.target.result);
            reader.onerror = (e) => reject(new Error("No se pudo leer el archivo. Intenta seleccionarlo de nuevo."));
        ...
        if (finalZip.files[documentFile]) {
            ...
            // Obtener colores
            const colorTitle = document.getElementById('colorTitle') ? document.getElementById('colorTitle').value.substring(1) : '2F5496';
            const colorLetters = document.getElementById('colorLetters') ? document.getElementById('colorLetters').value.substring(1) : '000000';
            const colorBorders = document.getElementById('colorBorders') ? document.getElementById('colorBorders').value.substring(1) : '000000';

            // Extraemos el contenido interno del template original (la "Guía")
            let templateBodyContent = "";
            const bodyStartIndex = xmlContent.indexOf('<w:body>') + '<w:body>'.length;
            const bodyEndIndex = xmlContent.indexOf('</w:body>');

            if (bodyStartIndex !== -1 && bodyEndIndex !== -1) {
                templateBodyContent = xmlContent.substring(bodyStartIndex, bodyEndIndex);
            }

            let guideContent = templateBodyContent;
            let sectPrContent = "";
            const sectPrIndex = templateBodyContent.lastIndexOf('<w:sectPr');
            if (sectPrIndex !== -1) {
                guideContent = templateBodyContent.substring(0, sectPrIndex);
                sectPrContent = templateBodyContent.substring(sectPrIndex);
            }

            const includeGuideCheckbox = document.getElementById('includeGuideCheck');
            const includeGuide = includeGuideCheckbox ? includeGuideCheckbox.checked : false;

            let finalBodyXML = "";

            if (includeGuide) {
                // Repetimos todo: [Guía] + [Salto] + [Crucigrama] + [Salto] + [Guía]...
                finalBodyXML = generateContentXML(students, searchWords, displayWords, currentDiff, seed, colorTitle, colorLetters, colorBorders, guideContent);
            } else {
                // Comportamiento Clásico: Dejar la Guía 1 vez al inicio, y anexar las sopas
                const sopasXML = generateContentXML(students, searchWords, displayWords, currentDiff, seed, colorTitle, colorLetters, colorBorders, "");
                const firstBreak = guideContent.trim() !== "" ? '<w:p><w:r><w:br w:type="page"/></w:r></w:p>' : '';
                finalBodyXML = guideContent + firstBreak + sopasXML;
            }

            // Asegurarnos de que el sectPr quede exclusivamente al final del documento
            finalBodyXML += sectPrContent;

            // Reemplazamos el contenido antiguo del body por el nuevo contenido
            xmlContent = xmlContent.substring(0, bodyStartIndex) + finalBodyXML + xmlContent.substring(bodyEndIndex);
...
function generateContentXML(students, searchWords, displayWords, difficulty, baseSeed, colorTitle = '2F5496', colorLetters = '000000', colorBorders = '000000', templateBodyContent = "") {
    let contentXML = '';
    ...
    for (let i = 0; i < students.length; i++) {
        ...
        // Generar grid única para cada estudiante usando una semilla diferente basada en su índice
        const grid = generateGrid(searchWords, baseSeed + i * 9999);
        ...
        // --- INYECCIÓN DE LA GUÍA (TEMPLATE) ---
        if (templateBodyContent) {
            // Word requiere que los IDs de párrafos, anclajes y dibujos sean ÚNICOS.
            let studentGuideContent = templateBodyContent;

            const randHex = () => Math.random().toString(16).slice(2, 10).toUpperCase();

            studentGuideContent = studentGuideContent.replace(/w14:paraId="[A-F0-9]+"/gi, () => `w14:paraId="${randHex()}"`);
            studentGuideContent = studentGuideContent.replace(/w14:textId="[A-F0-9]+"/gi, () => `w14:textId="${randHex()}"`);
            studentGuideContent = studentGuideContent.replace(/wp14:anchorId="[A-F0-9]+"/gi, () => `wp14:anchorId="${randHex()}"`);
            studentGuideContent = studentGuideContent.replace(/wp14:editId="[A-F0-9]+"/gi, () => `wp14:editId="${randHex()}"`);

            let idMap = {};
            studentGuideContent = studentGuideContent.replace(/\bid="(\d+)"/gi, (match, p1) => {
                if (!idMap[p1]) idMap[p1] = Math.floor(Math.random() * 10000000) + 1000;
                return `id="${idMap[p1]}"`;
            });

            contentXML += studentGuideContent;
            // Salto de página después de la guía, para que la sopa quede en la hoja siguiente
            contentXML += '<w:p><w:r><w:br w:type="page"/></w:r></w:p>';
        }

        // --- SECCIÓN: HEADER COMPACTO (Estudiante + Título en misma línea) ---
        // Usamos una tabla de 2 columnas: Izq (Estudiante), Der (Título)
        contentXML += '<w:tbl><w:tblPr><w:tblW w:w="0" w:type="auto"/><w:tblBorders><w:top w:val="none"/><w:left w:val="none"/><w:bottom w:val="none"/><w:right w:val="none"/><w:insideH w:val="none"/><w:insideV w:val="none"/></w:tblBorders></w:tblPr><w:tblGrid><w:gridCol w:w="6000"/><w:gridCol w:w="3500"/></w:tblGrid><w:tr><w:tc><w:tcPr><w:tcW w:w="6000" w:type="dxa"/><w:vAlign w:val="bottom"/></w:tcPr><w:p><w:r><w:rPr><w:b/><w:sz w:val="' + subtitleSize + '"/></w:rPr><w:t xml:space="preserve">Student: </w:t></w:r><w:r><w:rPr><w:sz w:val="' + subtitleSize + '"/><w:u w:val="single"/></w:rPr><w:t xml:space="preserve">  ' + student + '  </w:t></w:r></w:p></w:tc><w:tc><w:tcPr><w:tcW w:w="3500" w:type="dxa"/><w:vAlign w:val="bottom"/></w:tcPr><w:p><w:pPr><w:jc w:val="right"/></w:pPr><w:r><w:rPr><w:b/><w:caps/><w:color w:val="' + colorTitle + '"/><w:sz w:val="' + titleSize + '"/></w:rPr><w:t>Sopa de Letras</w:t></w:r></w:p></w:tc></w:tr></w:tbl>';
        ...
        contentXML += '<w:tbl><w:tblPr><w:tblW w:w="0" w:type="auto"/><w:jc w:val="center"/><w:tblBorders><w:top w:val="single" w:sz="12" w:color="' + colorBorders + '"/><w:left w:val="single" w:sz="12" w:color="' + colorBorders + '"/><w:bottom w:val="single" w:sz="12" w:color="' + colorBorders + '"/><w:right w:val="single" w:sz="12" w:color="' + colorBorders + '"/><w:insideH w:val="single" w:sz="4" w:color="' + colorBorders + '"/><w:insideV w:val="single" w:sz="4" w:color="' + colorBorders + '"/></w:tblBorders></w:tblPr><w:tblGrid>';
        ...
        for (let r = 0; r < gridSize; r++) {
            ...
            for (let c = 0; c < gridSize; c++) {
                ...
                contentXML += `<w:tc><w:tcPr><w:tcW w:w="${cellSize}" w:type="dxa"/><w:vAlign w:val="center"/></w:tcPr><w:p><w:pPr><w:jc w:val="center"/><w:spacing w:after="0" w:before="0"/></w:pPr><w:r><w:rPr><w:b/><w:color w:val="${colorLetters}"/><w:sz w:val="${gridFontSize}"/><w:szCs w:val="${gridFontSize}"/></w:rPr><w:t>${letter}</w:t></w:r></w:p></w:tc>`;
            }
            contentXML += '</w:tr>';
        }
        contentXML += '</w:tbl>';
