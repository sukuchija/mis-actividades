document.addEventListener('DOMContentLoaded', () => {
    ...
    const toggleAnswersBtn = document.getElementById('toggleAnswersBtn'); // Toggle Answers button
    ...
    const modeRadios = document.querySelectorAll('input[name="puzzleMode"]');
    const wordsInstruction = document.getElementById('wordsInstruction');
    ...
    const defaultText = `ECUACION: Igualdad entre dos expresiones que contiene una o más variables.
VARIABLE: Símbolo que representa un número desconocido.
ALGEBRA: Rama de las matemáticas que usa letras para representar relaciones.
SUMA: Operación aritmética que consiste en reunir varias cantidades.
RESTA: Operación inversa a la suma.
GRAFICA: Representación visual de datos o funciones.
ANGULO: Figura formada por dos rayos que comparten un punto final común.
TRIANGULO: Polígono de tres lados.
POTENCIA: Operación que consiste en multiplicar un número por sí mismo varias veces.
RAIZ: Operación inversa a la potencia.`;

    const defaultMath = `12: x + 2 = 14
25: 5 * 5
10: 5 + 5
50: 100 / 2
20: 10 * 2
...
40: 20 + 20`;
    ...
    modeRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
            if (e.target.value === 'math') {
                wordsInstruction.innerHTML = 'Ingresa el resultado y la ecuación/operación. Formato: <strong>Resultado: Ecuación o Polinomio</strong> (ej. 12: x+2=14). También puedes escribir operaciones básicas directamente (ej. 5+5).';
                wordsInput.value = defaultMath;
            } else {
                wordsInstruction.innerHTML = 'Ingresa las palabras y sus pistas. Formato: <strong>Palabra: Pista</strong> (una por línea).';
                wordsInput.value = defaultText;
            }
        });
    });
    ...
    function generatePuzzles() {
        const puzzleMode = document.querySelector('input[name="puzzleMode"]:checked').value;
        ...
        const wordList = rawWords.map(line => {
            ...
            if (puzzleMode === 'math') {
                if (parts.length > 1) {
                    const word = parts[0].trim().toUpperCase().replace(/[^A-Z0-9.\-]/g, '');
                    // parts[0] is the result. The rest of the string after the FIRST colon is the equation.
                    const firstColonIndex = line.indexOf(':');
                    const clue = line.substring(firstColonIndex + 1).trim();
                    return { word, clue };
                } else {
                    // Try to evaluate basic math
                    try {
                        let expr = line.trim().replace(/x/gi, '*');
                        let result = Function('"use strict";return (' + expr + ')')();
                        if (typeof result === 'number' && !isNaN(result)) {
                            return { word: result.toString().replace(/[^0-9.\-]/g, ''), clue: line.trim() };
                        }
                    } catch (e) { }
                    return null; // Invalid or un-evaluable line
                }
            } else {
                const word = parts[0] ? parts[0].trim().toUpperCase().replace(/[^A-ZÁÉÍÓÚÑ]/g, '') : '';
                const clue = parts.length > 1 ? parts.slice(1).join(':').trim() : 'Sin pista';
                return { word, clue };
            }
        }).filter(item => item && item.word.length > 0);
    ...
    function renderPuzzle(studentName, gridData, puzzleId) {
        ...
        for (let r = minR; r <= maxR; r++) {
            for (let c = minC; c <= maxC; c++) {
                ...
                if (cell) {
                    gridHtml += `<div class="grid-cell"><span class="cell-number">${number}</span><span class="cell-char">${cell.char}</span></div>`;
                } else {
    ...
    async function downloadWord() {
        ...
        try {
            ...
            if (finalZip.files[documentFile]) {
                ...
                // El XML de Word usualmente tiene <w:body> ... contenido ... <w:sectPr>...</w:sectPr></w:body>
                // Para simplificar, buscamos todo lo que está dentro de <w:body> excluyendo sectPr si es posible p ara evitar saltos raros al final, o simplemente copiar el contenido entero asumiendo estructrur normal.
                ...
                // Generamos todo el XML final repetido por estudiante: [Guía] + [Salto] + [Crucigrama] + [Salto] + [Guía]...
                const finalBodyXML = generateCrosswordContentXML(currentPuzzlesData, puzzleTitle, instructions, templateBodyContent);

                // Reemplazamos el contenido antiguo del body por el nuevo contenido repetido
    ...
    function generateCrosswordContentXML(puzzlesData, puzzleTitle, instructions, templateBodyContent = "") {
        let contentXML = '';
        ...
        puzzlesData.forEach((data, index) => {
            ...
            // Salto de página para el siguiente estudiante (sólo si no es el primer elemento O si acabamos de insertar un templateBodyContent)
            // En nuestra nueva lógica, SIEMPRE insertaremos el template.
            if (index > 0) {
            ...
            if (templateBodyContent) {
                contentXML += templateBodyContent;
                // Salto de página después de la guía, para que el crucigrama quede en la hoja siguiente
                contentXML += '<w:p><w:r><w:br w:type="page"/></w:r></w:p>';
            }
